...
export default function AdminReferralsScreen() {
  ...
  async function handleAddTier() {
    ...
    if (!Number.isFinite(min) || min < 1) {
      Alert.alert("Mínimo de referidos inválido");
      return;
    }

    if (!Number.isFinite(bonus) || bonus < 0) {
      Alert.alert("Bonus inválido");
      return;
    }
    ...
    <View style={styles.page}>
      ...
      <AppCard>
        ...
        <Text style={styles.cardLine}>
          Recompensa: % configurable sobre ganancias reales del profesional referido.
        </Text>
        <Text style={styles.cardLine}>
          Cada transacción elegible genera un evento de reward auditado individualmente.
        </Text>
        <Text style={styles.cardLine}>
          Créditos promocionales y regalos admin NO generan recompensas.
        </Text>
      </AppCard>
      ...
      <AppCard>
        ...
        <Text style={styles.cardLine}>
          El porcentaje adicional se suma al base cuando el referente supera el mínimo de
          referidos activos.
        </Text>
        ...
        <View style={styles.tierForm}>
          ...
          <Pressable
            style={[styles.addBtn, savingTier && { opacity: 0.6 }]}
            disabled={savingTier}
      ...
      {rows.length === 0 ? (
        <AdminEmptyState
          title="Sin referidos"
          description="Aún no hay actividad del programa de referidos."
        />
      ) : (
        <AdminDataTable
          rows={rows}
          columns={[
            {
              ...
              title: "Código",
              width: 140,
              render: (row) => <Text style={styles.cellPrimary}>{row.codeUsed}</Text>,
            },
            {
              key: "referred",
              ...
              render: (row) => (
                <Text style={styles.cellPrimary}>
                  {row.referred.fullName || row.referred.email || "Usuario"}
                </Text>
              ),
            },
            {
              ...
              render: (row) => (
                <Text style={styles.cellMuted}>
                  {row.referrer.fullName || row.referrer.email || "Usuario"}
                </Text>
              ),
            },
            {
            ...
            },
            {
              ...
              render: (row) => (
                <Text style={styles.cellPrimary}>
                  {Number(row.rewardCredits).toFixed(2)} cr
                </Text>
              ),
            },
            {
